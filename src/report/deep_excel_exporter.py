"""Deep Excel model exporter.

Creates a more complete analyst-style workbook with:
- Output / football field summary
- Source data
...
- Sector engines: healthcare rNPV, mining NAV, SaaS unit economics, financials ROE/P/B

The goal is not to pretend the workbook is institutionally audited. It is a
structured model scaffold with formulas, source-data placeholders, input cells,
and valuation engines that can be reviewed and refined by an analyst.
"""
...
def _headers(ws, row: int, labels: list[str], start_col: int = 1) -> None:
...
def _hist_years(report: FullReport) -> list[str]:
    years = sorted(report.historicals.revenue.keys())[-3:]
    return years or ["Hist"]
...
def _base_assumption(report: FullReport):
    return report.assumptions.get("base")
...
def _create_source_data(wb: Workbook, report: FullReport) -> None:
    ...
    _style_sheet(ws)
    _title(ws, "Source Data and Historical Financials", 12)
    ...
    for i, (metric, values) in enumerate(metrics, start=r + 1):
        ...
        for j, year in enumerate(years, start=2):
            _input(ws.cell(i, j), values.get(year), f"Historical {metric}; source: yfinance / filings review required")
    ...
    last_ebit = _last_value(report.historicals.ebit, _safe(report.snapshot.ebitda, 0) * 0.8)
    ...
    bs_rows = [
        ("Cash", opening_cash, "actual yfinance cash if available; otherwise estimated"),
        ...
        ("Debt", opening_debt, "actual yfinance debt if available"),
        ("Other Liabilities", opening_other_liab, "estimated placeholder"),
        ("Equity", opening_equity, "plugged opening book equity so BS balances"),
    ]
...
def _create_assumptions(wb: Workbook, report: FullReport) -> None:
    ...
    rows = [
        ...
        ["DPO"] + [50] * len(years),
        ["Other Assets % Revenue"] + [0.10] * len(years),
        ["Other Liabilities % Revenue"] + [0.12] * len(years),
    ...
    pct_rows = [4, 5, 6, 7, 8, 12, 13, 14, 15, 16, 17, 18, 19]
    for row in pct_rows:
        for col in range(2, 2 + len(years)):
            ws.cell(row, col).number_format = PCT_FMT
    for row in [9, 10, 11]:
    ...
    for col in range(2, 2 + len(years)):
        ws.cell(20, col).number_format = MULT_FMT
...
def _create_operating_schedules(wb: Workbook, report: FullReport) -> None:
    ...
    for col in range(2, 2 + len(years)):
        ...
        _formula(ws.cell(start + 1, col), f"={c}{start}*(1-Assumptions!{c}$5)")
        _formula(ws.cell(start + 2, col), f"={c}{start}*Assumptions!{c}$9/365")
        _formula(ws.cell(start + 3, col), f"={c}{start+1}*Assumptions!{c}$10/365")
        _formula(ws.cell(start + 4, col), f"={c}{start+1}*Assumptions!{c}$11/365")
        ...
        _link(ws.cell(start + 8, col), f"='3 Statement Model'!{c}24")
        _link(ws.cell(start + 9, col), f"='3 Statement Model'!{c}9")
...
def _create_three_statement(wb: Workbook, report: FullReport) -> None:
    ...
    years = _forecast_years(report)
    r = 3
    r = _section(ws, r, "Income Statement", 8)
    _headers(ws, r, ["Metric"] + years)
    ...
    for col in range(2, 2 + len(years)):
        ...
        p = get_column_letter(col - 1)
        if col == 2:
            _formula(ws.cell(start, col), f"='Source Data'!{get_column_letter(1+len(_hist_years(report)))}24*(1+Assumptions!{c}$4)")
        else:
            _formula(ws.cell(start, col), f"={p}{start}*(1+Assumptions!{c}$4)")
        _link(ws.cell(start + 1, col), f"=Assumptions!{c}$4")
        _formula(ws.cell(start + 2, col), f"={c}{start}*Assumptions!{c}$5")
        _formula(ws.cell(start + 3, col), f"={c}{start+2}/{c}{start}")
        _formula(ws.cell(start + 4, col), f"={c}{start}*Assumptions!{c}$6")
        _formula(ws.cell(start + 5, col), f"={c}{start+4}/{c}{start}")
        _formula(ws.cell(start + 6, col), f"={c}{start}*Assumptions!{c}$7")
        _formula(ws.cell(start + 7, col), f"={c}{start+4}-{c}{start+6}")
        if col == 2:
            _formula(ws.cell(start + 8, col), f"=AVERAGE('Source Data'!$B$44,{c}30)*Assumptions!{c}$15")
        else:
            _formula(ws.cell(start + 8, col), f"=AVERAGE({p}30,{c}30)*Assumptions!{c}$15")
        _formula(ws.cell(start + 9, col), f"={c}{start+7}-{c}{start+8}")
        _formula(ws.cell(start + 10, col), f"=MAX({c}{start+9},0)*Assumptions!{c}$14")
        _formula(ws.cell(start + 11, col), f"={c}{start+9}-{c}{start+10}")

    r = 19
    r = _section(ws, r, "Balance Sheet", 8)
    _headers(ws, r, ["Metric"] + years)
    ...
    for col in range(2, 2 + len(years)):
        c = get_column_letter(col)
        p = get_column_letter(col - 1)
        # Cash linked from CFS closing cash.
        _link(ws.cell(bs, col), f"={c}47")
        ...
        _formula(ws.cell(bs + 4, col), f"={c}{start}*Assumptions!{c}$12")
        _formula(ws.cell(bs + 5, col), f"=SUM({c}{bs}:{c}{bs+4})")
        _link(ws.cell(bs + 6, col), f"='Operating Schedules'!{c}9")
        if col == 2:
            _formula(ws.cell(bs + 7, col), f"='Source Data'!$B$44*(1-Assumptions!{c}$16)")
        else:
            _formula(ws.cell(bs + 7, col), f"={p}{bs+7}*(1-Assumptions!{c}$16)")
        _formula(ws.cell(bs + 8, col), f"={c}{start}*Assumptions!{c}$13")
        _formula(ws.cell(bs + 9, col), f"=SUM({c}{bs+6}:{c}{bs+8})")
        if col == 2:
            _formula(ws.cell(bs + 10, col), f"='Source Data'!$B$46+{c}{start+11}-{c}46")
        else:
            _formula(ws.cell(bs + 10, col), f"={p}{bs+10}+{c}{start+11}-{c}46")
        _formula(ws.cell(bs + 11, col), f"={c}{bs+9}+{c}{bs+10}")
        _formula(ws.cell(bs + 12, col), f"={c}{bs+5}-{c}{bs+11}")

    r = 36
    r = _section(ws, r, "Cash Flow Statement", 8)
    _headers(ws, r, ["Metric"] + years)
    cf_rows = ["Net Income", "D&A", "Change in NWC", "Cash Flow from Operations", "Capex", "Debt Repayment", "Dividends / Buybacks", "Net Change in Cash", "Ending Cash", "Free Cash Flow"]
    for i, label in enumerate(cf_rows, start=r + 1):
        ws.cell(i, 1, label)
    cf = r + 1
    for col in range(2, 2 + len(years)):
        c = get_column_letter(col)
        _link(ws.cell(cf, col), f"={c}{start+11}")
        _link(ws.cell(cf + 1, col), f"={c}{start+6}")
        _link(ws.cell(cf + 2, col), f"='Operating Schedules'!{c}11")
        _formula(ws.cell(cf + 3, col), f"={c}{cf}+{c}{cf+1}-{c}{cf+2}")
        _formula(ws.cell(cf + 4, col), f"=-{c}{start}*Assumptions!{c}$8")
        if col == 2:
            _formula(ws.cell(cf + 5, col), f"=-'Source Data'!$B$44*Assumptions!{c}$16")
        else:
            _formula(ws.cell(cf + 5, col), f"=-{get_column_letter(col-1)}30*Assumptions!{c}$16")
        _formula(ws.cell(cf + 6, col), f"=-{c}{start+11}*Assumptions!{c}$17")
        _formula(ws.cell(cf + 7, col), f"={c}{cf+3}+{c}{cf+4}+{c}{cf+5}+{c}{cf+6}")
        if col == 2:
            _formula(ws.cell(cf + 8, col), f"='Source Data'!$B$38+{c}{cf+7}")
        else:
            _formula(ws.cell(cf + 8, col), f"={get_column_letter(col-1)}{cf+8}+{c}{cf+7}")
        _formula(ws.cell(cf + 9, col), f"={c}{cf+3}+{c}{cf+4}")

    # Formatting.
    for row in list(range(start, start + len(is_rows))) + list(range(bs, bs + len(bs_rows))) + list(range(cf, cf + len(cf_rows))):
        for col in range(2, 2 + len(years)):
            ws.cell(row, col).number_format = NUM_FMT
    for row in [start + 1, start + 3, start + 5]:
        for col in range(2, 2 + len(years)):
            ws.cell(row, col).number_format = PCT_FMT
...
def _create_wacc(wb: Workbook, report: FullReport) -> None:
    ...
    rows = [
        ...
        ["Cost of Equity", "=B4+B5*B6+B7", "Formula"],
    ]
    ...
    rows = [
        ...
        ["Total Capital", "=B12+B13", "Formula"],
        ["Equity Weight", "=B12/B14", "Formula"],
        ["Debt Weight", "=B13/B14", "Formula"],
        ...
        ["After-tax Cost of Debt", "=B17*(1-B18)", "Formula"],
        ["WACC", "=B15*B8+B16*B19", "Formula"],
    ]
    _headers(ws, r, ["Item", "Value", "Comment"])
    for i, row in enumerate(rows, start=r + 1):
        ws.cell(i, 1, row[0])
...
def _create_dcf(wb: Workbook, report: FullReport) -> None:
    ...
    rows = ["FCF", "Discount Factor", "PV of FCF", "Terminal Value", "PV of Terminal Value", "Enterprise Value", "Net Debt", "Equity Value", "Shares", "Target Price"]
    for i, label in enumerate(rows, start=r + 1):
        ws.cell(i, 1, label)
    start = r + 1
    for col in range(2, 2 + len(years)):
        c = get_column_letter(col)
        _link(ws.cell(start, col), f"='3 Statement Model'!{c}46")
        _formula(ws.cell(start + 1, col), f"=1/(1+WACC!$B$20)^{col-1}")
    ...
    _formula(ws.cell(start + 3, term_col), f"={last}{start}*(1+Assumptions!{last}$19)/(WACC!$B$20-Assumptions!{last}$19)")
    _formula(ws.cell(start + 4, term_col), f"={term}{start+3}/(1+WACC!$B$20)^{len(years)}")
    _formula(ws.cell(start + 5, term_col), f"=SUM(B{start+2}:{last}{start+2})+{term}{start+4}")
    _link(ws.cell(start + 6, term_col), "='Source Data'!$B$14")
    _formula(ws.cell(start + 7, term_col), f"={term}{start+5}-{term}{start+6}")
    _link(ws.cell(start + 8, term_col), "='Source Data'!$B$11")
...
def _create_comps(wb: Workbook, report: FullReport) -> None:
    ...
    for c in range(3, 7): ws.cell(row, c).number_format = NUM_FMT
    ws.cell(row, 7).number_format = MULT_FMT
    ws.cell(row, 8).number_format = MULT_FMT
    ws.cell(row, 9).number_format = MULT_FMT
    r = row + 3
    r = _section(ws, r, "Implied Valuation", 8)
    rows = [
        ["Selected EV/EBITDA", "=H4", "Input linked to subject until peer set is populated"],
        ["Forward EBITDA", "='3 Statement Model'!B9", "Next forecast year EBITDA"],
        ["Implied EV", "=B8*B9", "Formula"],
        ["Less Net Debt", "='Source Data'!B14", "Source data"],
        ["Implied Equity Value", "=B10-B11", "Formula"],
        ["Shares", "='Source Data'!B11", "Source data"],
        ["Implied Price", "=B12/B13", "Formula"],
    ]
    _headers(ws, r, ["Metric", "Value", "Comment"])
    for i, row_data in enumerate(rows, start=r + 1):
        ws.cell(i, 1, row_data[0])
        if isinstance(row_data[1], str) and row_data[1].startswith("="):
            _formula(ws.cell(i, 2), row_data[1])
        else:
            _input(ws.cell(i, 2), row_data[1], row_data[2])
        ws.cell(i, 3, row_data[2])
    ws["B8"].number_format = MULT_FMT
    for row in range(9, 14): ws.cell(row, 2).number_format = NUM_FMT
    ws["B14"].number_format = PRICE_FMT
...
def _create_sensitivity(wb: Workbook, report: FullReport) -> None:
    ...
    for i, w in enumerate(waccs, start=r + 1):
        ...
        for j, tg in enumerate(tgs, start=2):
            # Linked sensitivity formula using DCF final year FCF and source data.
            last_fcf = f"'3 Statement Model'!{get_column_letter(1+len(_forecast_years(report)))}46"
            ws.cell(i, j, f"=(({last_fcf}*(1+{tg})/({w}-{tg}))/(1+{w})^{len(_forecast_years(report))}+SUM(DCF!B6:{get_column_letter(1+len(_forecast_years(report)))}6)-'Source Data'!$B$14)/'Source Data'!$B$11")
...
def _create_football_field(wb: Workbook, report: FullReport) -> None:
    ...
    sector = target
    if report.company_classification:
        if report.company_classification.primary_type == "healthcare_pipeline_or_royalty": sector = target or current
        elif report.company_classification.primary_type == "resources_or_mining": sector = current
        elif report.company_classification.primary_type in {"bank_or_lender", "insurer"}: sector = target or current
        elif report.company_classification.primary_type == "software_or_saas": sector = target or current
    r = 3
...
def _create_healthcare_rnpv(wb: Workbook, report: FullReport) -> None:
    ws = wb.create_sheet("Healthcare rNPV")
    _style_sheet(ws)
    _title(ws, "Healthcare Pipeline rNPV Engine", 14)
    r = 3
    ...
    assets = [
        ["Asset 1", "Lead indication", "Phase 2", 0.30, 2028, 500_000_000, 0.35, 5, 0.12],
        ["Asset 2", "Follow-on", "Phase 1", 0.15, 2030, 300_000_000, 0.30, 6, 0.14],
        ["Asset 3", "Preclinical", "Preclinical", 0.08, 2031, 200_000_000, 0.25, 7, 0.16],
    ]
    for i, asset in enumerate(assets, start=r + 1):
        for j, value in enumerate(asset, start=1):
            _input(ws.cell(i, j), value, "Pipeline assumption; replace with company-specific source evidence and Tufts/DiMasi-style PoS where applicable.")
        _formula(ws.cell(i, 10), f"=F{i}*G{i}/I{i}")
        _formula(ws.cell(i, 11), f"=J{i}*D{i}")
        _formula(ws.cell(i, 12), f"=K{i}/'Source Data'!$B$11")
    for row in range(r + 1, r + 4):
        ws.cell(row, 4).number_format = PCT_FMT
        ws.cell(row, 6).number_format = NUM_FMT
        ws.cell(row, 7).number_format = PCT_FMT
        ws.cell(row, 9).number_format = PCT_FMT
        for col in [10, 11]: ws.cell(row, col).number_format = NUM_FMT
        ws.cell(row, 12).number_format = PRICE_FMT
    total_row = r + 5
    ws.cell(total_row, 1, "Total rNPV")
    _formula(ws.cell(total_row, 11), f"=SUM(K{r+1}:K{r+3})")
    _formula(ws.cell(total_row, 12), f"=K{total_row}/'Source Data'!$B$11")
    ws.cell(total_row, 11).number_format = NUM_FMT
    ws.cell(total_row, 12).number_format = PRICE_FMT


def _create_mining_nav(wb: Workbook, report: FullReport) -> None:
    ws = wb.create_sheet("Mining NAV")
    _style_sheet(ws)
    _title(ws, "Mining NAV Engine", 14)
    years = ["Y1", "Y2", "Y3", "Y4", "Y5", "Y6", "Y7", "Y8"]
    r = 3
    r = _section(ws, r, "Finite-Life Mine Cash Flow", 12)
    _headers(ws, r, ["Metric"] + years)
    ...
    for i, metric in enumerate(metrics, start=r + 1): ws.cell(i, 1, metric)
    start = r + 1
    for col in range(2, 2 + len(years)):
        c = get_column_letter(col)
        _input(ws.cell(start, col), 100_000, "Production assumption; replace with mine plan.")
        ...
        _input(ws.cell(start + 5, col), 5_000_000, "Sustaining capex assumption.")
        _input(ws.cell(start + 6, col), 0 if col < 2 + len(years) - 1 else 20_000_000, "Closure cost in final year.")
        _formula(ws.cell(start + 7, col), f"={c}{start+2}-{c}{start+4}-{c}{start+5}-{c}{start+6}")
        _formula(ws.cell(start + 8, col), f"=1/(1+WACC!$B$20)^{col-1}")
        _formula(ws.cell(start + 9, col), f"={c}{start+7}*{c}{start+8}")
    total_row = start + 11
    ws.cell(total_row, 1, "Project NAV")
    _formula(ws.cell(total_row, 2), f"=SUM(B{start+9}:{get_column_letter(1+len(years))}{start+9})")
    ws.cell(total_row, 2).number_format = NUM_FMT


def _create_saas_engine(wb: Workbook, report: FullReport) -> None:
    ws = wb.create_sheet("SaaS Unit Economics")
    _style_sheet(ws)
    _title(ws, "SaaS Unit Economics Engine", 12)
    years = _forecast_years(report)
    r = 3
    r = _section(ws, r, "ARR and Unit Economics", 10)
    _headers(ws, r, ["Metric"] + years)
    metrics = ["Opening ARR", "New ARR", "Churned ARR", "Ending ARR", "ARR Growth", "Revenue", "Gross Margin", "Sales & Marketing", "Rule of 40", "EV/ARR Implied EV", "Implied Price"]
    for i, metric in enumerate(metrics, start=r + 1): ws.cell(i, 1, metric)
    start = r + 1
    for col in range(2, 2 + len(years)):
        c = get_column_letter(col)
        p = get_column_letter(col - 1)
        if col == 2:
            _input(ws.cell(start, col), _safe(report.snapshot.revenue_ttm, 100_000_000), "Opening ARR proxy; replace with company ARR disclosure.")
        else:
        ...
        _input(ws.cell(start + 7, col), 0.35, "Sales & marketing as % revenue")
        _formula(ws.cell(start + 8, col), f"={c}{start+4}+('3 Statement Model'!{c}46/'3 Statement Model'!{c}5)")
        _input(ws.cell(start + 9, col), 8.0, "EV/ARR multiple")
        _formula(ws.cell(start + 10, col), f"={c}{start+3}*{c}{start+9}/'Source Data'!$B$11")
    for row in [start + 1, start + 2, start + 4, start + 6, start + 7, start + 8]:
        for col in range(2, 2 + len(years)): ws.cell(row, col).number_format = PCT_FMT
    for row in [start, start + 3, start + 5]:
        for col in range(2, 2 + len(years)): ws.cell(row, col).number_format = NUM_FMT
    for col in range(2, 2 + len(years)):
        ws.cell(start + 9, col).number_format = MULT_FMT
        ws.cell(start + 10, col).number_format = PRICE_FMT


def _create_financials_engine(wb: Workbook, report: FullReport) -> None:
    ws = wb.create_sheet("Financials ROE-PBV")
    _style_sheet(ws)
    _title(ws, "Financials ROE / P-BV Engine", 10)
    r = 3
    ...
    rows = [
        ...
        ["Cost of Equity", "=WACC!B8", "Formula"],
        ["Long-term Growth", 0.03, "Input"],
        ["Justified P/B", "=(B5-B7)/(B6-B7)", "Formula"],
        ["Target Price", "=B4*B8", "Formula"],
    ]
    _headers(ws, r, ["Metric", "Value", "Comment"])
    for i, row in enumerate(rows, start=r + 1):
        ws.cell(i, 1, row[0])
        if isinstance(row[1], str): _formula(ws.cell(i, 2), row[1])
        else: _input(ws.cell(i, 2), row[1], row[2])
        ws.cell(i, 3, row[2])
    ws["B4"].number_format = PRICE_FMT
...
def export_excel_model(report: FullReport, output_dir: Path) -> str:
    ...
    # Include detailed sector engines. Analysts can use the relevant one based on the valuation router.
    _create_healthcare_rnpv(wb, report)
    _create_mining_nav(wb, report)
    _create_saas_engine(wb, report)
    _create_financials_engine(wb, report)

    _format_all(wb)
    path = output_dir / f"{report.snapshot.ticker.lower()}_{report.plan.report_type}_deep_model.xlsx"
    wb.save(path)
    return str(path)
